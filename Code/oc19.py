...
MAX_FRIENDS = 200000000
FRIENDS_OF_FRIENDS_LIMIT = 20000000
...
CONSUMER_KEY =        '3cVcJ8LGOKlS8V6nGqgEaoFxG'
CONSUMER_SECRET =     'M2XkcDPAZklAaYETnFRy06uXq0xqPlxJfgcjbDixsOqqmqMeuc'
ACCESS_TOKEN   =        '3219681132-x6BOXC5Gzj05r06LXUVsvMzTuHrlGnEo7eUsUI9'
ACCESS_TOKEN_SECRET =  'lUKYkoY3aXSTHc2BwBsEODEI1wD4kKWcAb6gQIkoyxaMs'
        ...
        if screen_name.startswith(''):
            if not os.path.exists(fname):
                ...
                with open(fname, 'w') as outf:
                    ...
                    while True:
                        try:
			    #print table['21457289']
                            friend = c.next()
                            friendids.append(friend.id)
			    #print table.keys()
			    if table.has_key(friend.id):
			    	op = table[friend.id]
				print op
			    else:
			    	op1 = api.get_user(friend.id)
				op = op1.followers_count					
                            	#print op
			    params = (friend.id, enc(friend.screen_name), enc(friend.name),op)
                            outf.write('%s\t%s\t%s\t%d\n' % params)
                            friend_count += 1
